    ...
    <nav class="stepped-process" aria-label="Checkout process">
      ...
      <ol>
        <li
          ...
          :class="{ active: currentStepIndex === index, complete: currentStepIndex > index }"
        >
          <a class="stepped-process-link" @click.prevent="setCurrentStepIndex(index)">{{
            Step.step
          }}</a>
        </li>
      </ol>
      ...
      <ol>
        <li
          ...
          :class="{
            ...
            complete: currentCategoryIndex > index
          }"
        >
          <a class="stepped-process-link" @click.prevent="setCurrentCategoryIndex(index)"></a>
        </li>
      </ol>
    </nav>
      ...
      <form class="my-4">
        ...
        <div v-for="(question, index) in currentCategory.questions" :key="index" class="mb-3">
          ...
          <div v-if="question.type === 'text'">
            <input type="text" :required="question.required" style="200 px" />
          </div>
  ...
  methods: {
    async fetchSteps() {
      ...
      for (let i = 0; i < this.steps.length; i++) {
        this.categories[this.steps[i].id] = this.steps[i].categories
      }
    ...
    setcurrentCategory() {
      const categoriesOfStep = this.categories[this.currentStepIndex]
      if (categoriesOfStep && categoriesOfStep.length > this.currentCategoryIndex + 1) {
        this.currentCategory = categoriesOfStep[this.currentCategoryIndex + 1]
      }
    }
  }
}
