...
export default {
  ...
  methods: {
    async fetchAssessments(){
      const response = await axios.get(`http://localhost:8088/api/v1/assessments`)
    ...
    async fetchSteps(id) {
      try {
        ...
        const response1 = await axios.get(
          `http://localhost:8088/api/v1/applications/${id}/assessment`
        )
        if (!response1.data) {
    ...
    LinkAssessment(id){
      const url = `http://localhost:8088/api/v1/applications/${this.$route.params.id}/addAssessment/${id}`
      axios
        .post(url)
